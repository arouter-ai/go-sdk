...
func main() {
	...
	// Test 1: OpenRouter via ChatCompletion (OpenAI-compatible)
	fmt.Println("[Test 1] OpenRouter - ChatCompletion (openrouter/google/gemini-2.0-flash-001)")
	testChatCompletion(client, "openrouter/google/gemini-2.0-flash-001")

	// Test 2: OpenRouter streaming
	fmt.Println("[Test 2] OpenRouter - Streaming ChatCompletion")
	testStreamingCompletion(client, "openrouter/google/gemini-2.0-flash-001")

	// Test 3: Direct proxy to OpenRouter
	fmt.Println("[Test 3] Direct proxy to openrouter")
...
func testProxyRequest(client *arouter.Client) {
	...
	resp, err := client.ProxyRequest(ctx, "openrouter", "v1/chat/completions", bytes.NewReader(payload))
	if err != nil {
		log.Printf("  FAIL: %v\n\n", err)
		return
	}
